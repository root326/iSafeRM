...
def my_cdnod(data: ndarray, c_indx: ndarray, alpha: float=0.05, indep_test: str=fisherz, stable: bool=True,
    ...
    if mvcdnod:
        ...
        return cdnod_alg(data=data_aug,c_indx=c_indx, alpha=alpha, indep_test=indep_test, stable=stable, uc_rule=uc_rule,
                         uc_priority=uc_priority, background_knowledge=background_knowledge, verbose=verbose,
                         show_progress=show_progress, **kwargs)


def cdnod_alg(data: ndarray, c_indx: ndarray, alpha: float, indep_test: str, stable: bool, uc_rule: int, uc_priority: int,
              background_knowledge: Optional[BackgroundKnowledge] = None, verbose: bool = False,
              show_progress: bool = True, **kwargs) -> CausalGraph:
    """
